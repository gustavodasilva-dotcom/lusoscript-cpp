...
| statement	  | → *exprStmt* \| *forStmt* \| *ifStmt* \| *imprimaStmt* \| *whileStmt* \| *block* ; |
| forStmt	  | → `para` `(` ( *varDecl* \| *exprStmt* \| `;` ) ( *expression* )? `;` ( *expression* )? `)` *statement* ; |
| ifStmt	  | → `se` `(` *expression* `)` *statement* ( `else` *statement* )? ; |
